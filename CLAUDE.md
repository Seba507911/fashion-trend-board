...
Market-organic은 선행 시그널 없이 소비자 수요에서 자연 발생. 발생 동인에 따라 세분화:
...
**모니터링 방법 차이**:
- Lifestyle-shift: 라이프스타일 외부 데이터(스포츠 참여율, 캠핑장 예약 등) 선행 지표
- Necessity-driven: 기상 데이터, 환경 지표 연동
- Event-triggered: 이벤트 발생 후 마켓 반응 속도 빠른 감지

**참고**: 현재 DB origin_type은 `market_organic` 단일값. 향후 `market_lifestyle`, `market_necessity`, `market_event` 세분화 검토 가능.
