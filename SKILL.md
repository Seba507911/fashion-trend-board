...
### Market-organic 서브타입 — 발생 동인별 3분류

Market-organic은 선행 시그널 없이 소비자 수요에서 자연 발생하는 트렌드로, 발생 동인에 따라 세분화:

```
Lifestyle-shift:    라이프스타일 변화 → 수요 생성 → 마켓 점진 확대 → 검색 완만 상승
                    (점진적, 정착 시 지속성 높음)
                    예) 캠핑붐→고프코어, 러닝인구증가→러닝화일상화, 자전거출퇴근→테크웨어

Necessity-driven:   외부 환경 변화 → 구매 강제 → 마켓 급증 → 환경 해소 시 정상화
                    (계절/환경 연동, 예측 상대적 용이, 일시적일 수 있음)
                    예) 한파→롱패딩, 폭염→냉감소재, 미세먼지→마스크패션화

Event-triggered:    예측 불가 외부 충격 → 소비 패턴 급변 → 마켓 급등 → 일부 정착/일부 소멸
                    (급격한 볼륨 변화, 이벤트 종료 후 이중 궤적)
                    예) 코로나→스웨트셋업/라운지웨어, 재택근무→컴포트웨어
```

**모니터링 방법 차이**:
- Lifestyle-shift: 라이프스타일 외부 데이터(스포츠 참여율, 캠핑장 예약 추이 등) 선행 지표
- Necessity-driven: 기상 데이터, 환경 지표 연동
- Event-triggered: 이벤트 발생 후 마켓 반응 속도 빠른 감지가 핵심

**참고**: 현재 DB의 origin_type은 `market_organic` 단일값. 향후 `market_lifestyle`, `market_necessity`, `market_event`로 세분화 검토 가능 (수동 태깅 또는 외부 데이터 연동 필요).
